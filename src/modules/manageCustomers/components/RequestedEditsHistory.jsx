...
import { getRequestEditsHistory } from '../../../redux/slice/manageCustomers/ManageCustomersSlice';
import CustKpiCards from './CustKpiCards';
import { useLocation } from 'react-router-dom';
import { REQUESTED_EDITS_HISTORY } from '../../../utils/enums/CustomersEnums';
import { RequestEditsHistoryContext } from '../ManageCustomers';

const RequestedEditsHistory = () => {
  const dispatch = useDispatch();
  const location = useLocation();
  const ReqEditHistory = useContext(RequestEditsHistoryContext);
  const { openRequestedEditsHistory, setOpenRequestedEditsHistory } =
  ReqEditHistory || {};
  const { rowDataUid } = location.state || {};
  const { currentEnv } = useSelector((state) => state?.authenticationSlice);
  const { requestedEditsHistoryDetails, reqCustomerLoader } = useSelector((state) => state?.manageCustomers);
  const { gridData, KpiDetails } = requestedEditsHistoryDetails || {};
  ...
  useEffect(() => {
    if (rowDataUid) {
      dispatch(getRequestEditsHistory(rowDataUid));
    }
  }, [rowDataUid]);

  useEffect(() => {
    return () => {
      setOpenRequestedEditsHistory && setOpenRequestedEditsHistory(false);
    };
  }, []);
  const columns = [
    {
    ...
    },
    {
      ...
      renderCell: (params) => {
        ...
        if (value) {
          ...
          formattedDate = date.toLocaleString('en-US', options).replace(',', '');
        }
        return (
          <Box
            sx={{
  ...
  return (
    <>
      <ASDrawer
        {...{
          open: openRequestedEditsHistory,
          setOpen: setOpenRequestedEditsHistory,
          action: 'Create',
          headerName: REQUESTED_EDITS_HISTORY,
          showFooter: false,
...
export default RequestedEditsHistory;
